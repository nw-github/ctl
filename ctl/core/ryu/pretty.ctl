...
unsafe fn write_mantissa_long(mut output: u64, mut res: *raw u8) {
    unsafe {
        if output >> 32 != 0 {
            ...
            output2 /= 10000;
            let d = output2 % 10000;
            core::mem::copy(dst: res - 2, src: &raw DIGIT_TABLE[((c % 100) << 1) as int], num: 2);
            core::mem::copy(dst: res - 4, src: &raw DIGIT_TABLE[((c / 100) << 1) as int], num: 2);
            core::mem::copy(dst: res - 6, src: &raw DIGIT_TABLE[((d % 100) << 1) as int], num: 2);
            core::mem::copy(dst: res - 8, src: &raw DIGIT_TABLE[((d / 100) << 1) as int], num: 2);
            res -= 8;
        }
...
unsafe fn write_mantissa(mut output: u32, mut res: *raw u8) {
    unsafe {
        ...
        if output >= 100 {
            let c = (output % 100) << 1;
            output /= 100;
            core::mem::copy(dst: res - 2, src: &raw DIGIT_TABLE[c as int], num: 2);
            res -= 2;
        }

        if output >= 10 {
            core::mem::copy(dst: res - 2, src: &raw DIGIT_TABLE[(output << 1) as int], num: 2);
        } else {
