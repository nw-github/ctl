    ...
    fn emit_type(
        ...
        match id {
            ...
            Type::User(ut) => match &scopes.get(ut.id).data {
                ...
                UserTypeData::Template => panic!("ICE: Template type in emit_type"),
...
impl<'a> Codegen<'a> {
    ...
    fn gen_ctl_init(&mut self) -> (String, String) {
        ...
        // not dangle
        let state = &mut self.find_function("core", "panic").unwrap();
    ...
    fn gen_fn(&mut self, state: &mut State, prototypes: &mut Buffer) {
        ...
        if let Some(body) = func.body.clone() {
            ...
            for param in func.params.iter() {
                ...
                self.gen_pattern_bindings(state, &patt.data, &param.label, &param.ty);
            }
            ...
            CheckedStmt::Let(patt, value) => {
                hoist_point!(self, {
                    if let CheckedPatternData::Variable(id) = patt.data {
                        ...
                        self.gen_pattern_bindings(state, &patt.data, &tmp, &ty);
                    }
                });
            }
            CheckedStmt::None => {}
        ...
        match expr.data {
            ...
            CheckedExprData::Call {
                ...
                if let Some(name) = self.scopes.intrinsic_name(func.id) {
                    return self.gen_intrinsic(name, &func, args, state);
                }
            ...
            CheckedExprData::Is(mut inner, patt) => {
                ...
                let tmp = hoist!(self, state, self.gen_tmpvar(*inner, state));
