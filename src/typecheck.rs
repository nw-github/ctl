        ...
        impl Scopes {
            ...
            concat_idents!(fn_name = insert_, $suffix, _in {
                pub fn fn_name(&mut self, item: $output, public: bool, scope: ScopeId) -> $name {
                    ...
                    let id = $name(index);
                    self[scope].$vec.insert(Vis { id, public });
                    id
                }
            });
...
impl ScopeKind {
    ...
    {
        match self {
            &ScopeKind::Function(id) => Some(&scopes.get_func(id).name.data),
            &ScopeKind::UserType(id) => Some(&scopes.get_user_type(id).name),
...
impl TypeChecker {
    ...
    fn check_stmt(&mut self, scopes: &mut Scopes, stmt: DeclaredStmt) -> CheckedStmt {
        match stmt.data {
            ...
            DeclaredStmtData::Struct { init, base } => {
                self.check_fn(scopes, init);
                scopes.enter_id(scopes.get_user_type(base.id).body_scope, |scopes| {
                    for id in base.type_params {
                        self.resolve_impls(scopes, id);
                    }

                    self.check_impl_blocks(scopes, base.id, base.impls);
                    for i in 0..scopes
                        .get_user_type(base.id)
                        .data
                        .as_struct()
                        .unwrap()
                        .0
            ...
            DeclaredStmtData::Static { id, value } => {
                ...
                let mut ty = scopes.get_var_mut(id).ty.clone();
                ty.resolve(scopes, self);
                (scopes.get_var_mut(id).ty) = ty.clone();
    ...
    fn resolve_type(&mut self, scopes: &Scopes, hint: &TypeHint, fwd: bool) -> TypeId {
        match hint {
            TypeHint::Regular(path) => {
                return match self.resolve_path(scopes, &path.data, path.span, fwd) {
            ...
            TypeHint::Option(ty) => {
                self.resolve_lang_type(scopes, "option", &[(**ty).clone()], fwd.then_some(hint))
            }
            TypeHint::Vec(ty) => {
                self.resolve_lang_type(scopes, "vec", &[(**ty).clone()], fwd.then_some(hint))
            }
            ...
            TypeHint::Set(ty) => {
                self.resolve_lang_type(scopes, "set", &[(**ty).clone()], fwd.then_some(hint))
            }
            TypeHint::Slice(ty) => {
                self.resolve_lang_type(scopes, "span", &[(**ty).clone()], fwd.then_some(hint))
            }
            TypeHint::SliceMut(ty) => {
                self.resolve_lang_type(scopes, "span_mut", &[(**ty).clone()], fwd.then_some(hint))
            }
    ...
    fn resolve_path(
        ...
        match path {
            ...
            Path::Normal(data) => {
                ...
                } else if let Some(id) = scopes.find_user_type(name) {
                    if is_end {
